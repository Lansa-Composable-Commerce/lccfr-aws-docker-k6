...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await axios.get(`${API.SUB_USER}/permissions`, config);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const config = {
      params: {
        subuserId: requestData?.subuserId,
      },
      data: requestData?.data,
    };

    const response = await axios.put(
      `${API.SUB_USER}/permissions?subuserId=${config?.params?.subuserId}`,
      config?.data,
    );
